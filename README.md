...
取得側は `khinsider-live-v2` に統一します。新ワークフローが取得した全ページの
アルバム一覧と、各アルバムの完全な曲一覧だけから `library.json` と曲名索引を
生成します。旧 `index.json`、過去の曲名キャッシュ、旧クロール成果から欠損を
補完する処理はありません。新経路自身の途中保存からの再開は行います。

- 未取得・曲一覧の解析失敗が残っている間は、新しい完成版を公開しません。
  HTTP 404 は取得成功ではなく、利用不可として別に報告します。
- 初回全量取得中は既存の公開済みデータを維持します。これはサービス継続用で、
  新しいデータを作る材料には使いません。既定の配布URLは変更しません。
- 新形式を名乗る library / manifest が不完全、または旧入力を含む場合は拒否し、
  最後に検証できたデータを保持します。不完全な新形式manifestを無視して
  未検証gzipへフォールバックすることもありません。
- 曲名索引は同じ4列TSV形式なので、取得完了後も既存の検索・Subsonic IDを使えます。
