...
取得側は `khinsider-live-v2` 自身の保存データを蓄積し、日々の新規・更新・
未取得分だけを差分収集します。全一覧の巡回は初期構築時のみで、毎日は行いません。
旧 `index.json`、廃止した処理の曲名キャッシュやクロール成果を混ぜて補完する
処理はありません。一方、新経路自身の取得済みデータは継続して再利用します。

- リリースは毎回、取得済みの有効なアルバム・曲をすべて含む全体版です。
  その日の差分だけで既存の索引を上書きすることはありません。
- 更新取得に失敗したアルバムは最後の正常データを保持し、失敗分だけを再試行します。
  初めて取得するアルバムが一部未取得でも、他の正常な更新の公開は止めません。
  HTTP 404 と、トップページへ転送されて取得できない状態は区別します。
- `complete: true` と `completeness_scope: cumulative_snapshot` は「蓄積全体を含む」
  意味です。全サイト取得完了とは別で、未取得・再試行件数は別途表示します。
- 初回の切り替えでは従来の曲数を下回る小規模索引への置換を防ぎます。
  過去の曲行を新しい索引の材料として読み込むわけではありません。
- 新形式の出所・ハッシュ・全体版の整合性を検証し、不正なデータでは最後の正常な
  library / SQLite を保持します。不正な新形式manifestを無視して未検証gzipへ
  フォールバックすることもありません。
- 既定の配布URL・4列TSV・検索・Subsonic IDの形式は維持します。ディスク番号0と、
  サイト上で拡張子が欠けた曲ページ名もそのまま扱い、古いアルバムキャッシュは
  次回アクセス時に新しい解析結果へ更新します。
  Unicodeを含むアルバムパスと曲リンクの照合を修正しました。修正前のアルバム
  解析キャッシュは、アクセス時に新しいキャッシュ版で取得し直します。
- 全件とは検証済み一覧の範囲です。サイトの変更を瞬時に反映する保証や、
  任意のpublisher/developer/date項目が必ず埋まるという意味ではありません。

**コード反映・全量取得・完成版公開・稼働中サーバーの更新確認は別の段階です。**
以下の325万曲等の数字や旧キャッシュ由来の精度測定は移行前の参考値であり、
新経路の全量取得が完了した証拠ではありません。新しい件数は配布manifestと
取得ワークフローのサマリーで確認してください。
